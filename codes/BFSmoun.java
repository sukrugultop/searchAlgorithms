...
//BFS algorithm for mountain problem
public class BFSmoun {
     ...
     *
     * If height between neighbour node(which can be 4 positions at max of current node which are up,left,right,down) and current node is not grater than 1 and not visited it adds to pq.
     *
    ...
    public String run(){
        ...
        while (!queue.isEmpty()){
            ...
            if (visited[currNode.x][currNode.y]==0){
                ...
                // adds the possible neighbours to open list.
                while (!pq.isEmpty()) {
                    Point p = pq.remove();
        ...
        //total distance traveled along the path which is euclidean distance.
